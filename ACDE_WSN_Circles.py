...
SAMPLES, _ = datasets.make_circles(n_samples=1000, shuffle=False, noise=0.1, random_state=3, factor=0.8)
...
def main_WSN(Dim):
    ...
    SAMPLES, _ = datasets.make_circles(n_samples=1000, shuffle=False, noise=0.1, random_state=3, factor=0.8)
    SAMPLES = Trunction(25 * (SAMPLES + 1), scales=[0, 50])
    ...
    np.savetxt("./ACDE_Data/WSN/Circle/Obj/WSN_" + str(int(Dim/2)) + ".csv", All_Trial_Best, delimiter=",")
    np.savetxt("./ACDE_Data/WSN/Circle/Sol/WSN_" + str(int(Dim/2)) + ".csv", All_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./ACDE_Data/WSN/Circle/Obj') == False:
        os.makedirs('./ACDE_Data/WSN/Circle/Obj')
    if os.path.exists('./ACDE_Data/WSN/Circle/Sol') == False:
        os.makedirs('./ACDE_Data/WSN/Circle/Sol')
    Dims = [32, 64, 84]
    for Dim in Dims:
        main_WSN(Dim)
