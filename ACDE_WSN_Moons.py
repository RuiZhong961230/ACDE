...
SAMPLES, _ = datasets.make_moons(n_samples=1000, shuffle=False, noise=0.15, random_state=1)
SAMPLES[:, 0] = (SAMPLES[:, 0] + 1) * 17
SAMPLES[:, 1] = (SAMPLES[:, 1] + 1) * 22
...
def main_WSN(Dim):
    ...
    np.savetxt("./ACDE_Data/WSN/Moons/Obj/WSN_" + str(int(Dim/2)) + ".csv", All_Trial_Best, delimiter=",")
    np.savetxt("./ACDE_Data/WSN/Moons/Sol/WSN_" + str(int(Dim/2)) + ".csv", All_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./ACDE_Data/WSN/Moons/Obj') == False:
        os.makedirs('./ACDE_Data/WSN/Moons/Obj')
    if os.path.exists('./ACDE_Data/WSN/Moons/Sol') == False:
        os.makedirs('./ACDE_Data/WSN/Moons/Sol')
    Dims = [32, 64, 84]
    for Dim in Dims:
        main_WSN(Dim)
